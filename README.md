...
The `genomedl` package exposes `genomedl.DownloadGenome(accession, outPath)` for downloading one genome accession. By default it writes an available annotation file (GFF3 with embedded FASTA, GenBank/GBFF, or EMBL), and falls back to FASTA when no annotation file is available. Use `genomedl.DownloadGenomeWithOptions` with `genomedl.DownloadOptions{Format: genomedl.GenomeFormatFASTA}` to force FASTA output, or `GenomeFormatGFF3` or `GenomeFormatEMBL` to request those formats.
