...
### Changed
- `faqt stats` output gains a `GC` column. In tab output it sits between `Gaps` and `N50`, so anything parsing those tables by column position needs updating; human and grep friendly output gain a `GC` line each.
...
- Buffer output written by `seqio.CreatePath` and `seqio.OpenWriter`, both above and below any compressor. Records were going straight to the file descriptor, so one FASTQ record cost five `write` syscalls and a wrapped FASTA line cost two; compressors then emitted their own small chunks straight to the descriptor too, which cost 28% extra on top when gzipping a 176 MB FASTA. `faqt to-fasta --wrap 60` on a 231 MB input drops from 10.1s to 0.22s. `Close` now flushes, so its error must be propagated for output to be complete. `seqio.NewWriter` stays unbuffered, because the caller owns the writer and may never call `Close`.
...
- Compress and decompress gzip with `klauspost/compress`, already a dependency: about 21% faster to read and 12% faster to write than `compress/gzip`. The compression level is pinned at 6, the level `compress/gzip` used by default, because klauspost defaults to 5 and would otherwise have grown every gzipped output by 2.7%. Gzip output is byte-identical to v0.9.1, as is bzip2, xz and zstd output.
...
### Added
- Report GC content from `faqt stats`, as a percentage of A+C+G+T. Ns, gaps and ambiguity codes are left out of the denominator, the convention `seqkit fx2tab --gc` uses, so Ns padding a draft assembly do not drag the reported GC down; a sequence of nothing but Ns reports `0.00`. Case is ignored and U counts as T.
- Add `faqt stats --per-sequence` (`-p`), which reports one tab-delimited row per sequence — `file`, `name`, `length`, `N_count`, `Gaps`, `GC` — instead of one result per input. The `file` column holds the input path, or `path:sample` for an AGC sample, so sequences sharing a name across inputs stay distinguishable. Rows stream as records are read, so inputs larger than memory are fine. `-l` filters short sequences as before and `-u` drops the header; `-s` and `--combine-inputs` are rejected, because both describe a whole input rather than one sequence.
- Add `stats.PerSequence`, `stats.WritePerSequence` and `stats.SeqStats` for per-sequence statistics, and `stats.Stats.GCPercent` for the aggregate figure.
- Add `seq.CountComposition`, which counts A, C, G, T, Ns and runs of Ns in one pass, with `Composition.ACGT` and `Composition.GCPercent` on the result. `stats` now counts through it rather than walking each sequence itself.
